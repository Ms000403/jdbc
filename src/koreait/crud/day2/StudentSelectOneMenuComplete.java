...
public class StudentSelectOneMenuComplete {
	...
	private static void selectOneStudent(Connection conn) {
		...
		try(
			...
			//조회된 rs 의 특정 컬럼값을 가져오기 할때, 컬럼의 데이터 타입을 확인하고
			//getXXXX메소드 정하기, getXXXX 메소드의 인자는 컬럼의 인덱스 또는 컬럼 이름입니다.
			...
			//결과 집합을 모두 소모했음 -> 조회 결과가 없는데 rs.getXXXX메소드 실행할 때의 예외 메세지
		}
		sc.close();
	}
