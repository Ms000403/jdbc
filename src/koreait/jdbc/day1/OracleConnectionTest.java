...
public class OracleConnectionTest {
	...
	public static void main(String[] args) {
		...
		//구현클래스가 있어야한다. db에서는 db드라이버가 접속하려는 db의 종류에 따라
		//알아서(프록시) 구현클래스와 구현 객체를 만든다.
		Connection conn = null;
		
		//1. 아래 4개의 필수 연결 정보를 설정한다.
		//접속하고자 하는 서버의 주소(포털접속)
		...
		//4. 3번의 결과로 오라클 db에 맞는 연결객체가 생성됩니다.
			System.out.println("연결상태 = " + conn);
			if(conn!=null) {
				System.out.println("comm 객체의 구현 클래스 : " + conn.getClass().getName());
				//oracle.jdbc.driver.T4CConnection 클래스로 객체가 생성
				
				System.out.println("오라클 데이터베이스 연결 성공 !!");
			}else
				System.out.println("오라클 데이터베이스 연결 실패 !!");
		}catch(Exception e) { //ClassNotFoundException, SQLException 처리필요
 ...
 * jdbc : 자바와 dbms를 연결하는 api. 오라클은 ojdbcx.jar이고 x는 오라클 jdbc 버전 표시
 * 
 */
